...
def read_login_entries():
    entered_given_name = login_given_name_entry.get()
    entered_surname = login_surname_entry.get()
    entered_password = login_password_entry.get()
    process_login(entered_given_name, entered_surname, entered_password)

def process_login(entered_given_name, entered_surname, entered_password):
...
def clear_login_page():
    entry_reseter([
        login_given_name_entry, login_surname_entry,
...
def clear_signup_page():
    given_name_entry.delete(0, tk.END)
...
def has_role_student(event):
    ...
    if current_account_role == "Student":
        grid_layout_manager([
            (grade_year_label, 5, 0, 10, 5, ""),
            (grade_level_selector, 5, 1, 10, 5, "")
        ])
...
def show_section_selector(event):
    ...
    if selected_grade in sections:
        ...
        grid_layout_manager([
            (grade_section_label, 6, 0, 10, 5, ""),
            (grade_section_selector, 6, 1, 10, 5, "")
        ])
...
start_menu_back_button = button_constructor(start_menu, "Back", "NORMAL", "TButton", lambda: show_frame(logged_out))

start_menu_widgets = [
    (signup_button, 1, 0, 10, 5, ""),
    (login_button, 2, 0, 10, 5, ""),
    (start_menu_back_button, 3, 0, 15, 5, "")
]
...
account_role_label = label_constructor(signup_page, "Select account role: ")
possible_account_roles = ("Student", "Teacher")
account_role_selector = ttk.Combobox(signup_page, textvariable=selected_account_role, values=possible_account_roles, state='readonly')

account_role_selector.bind("<<ComboboxSelected>>", has_role_student)

# Grade Yr. and Section (If applicable)
grade_year_label = label_constructor(signup_page, "Select Grade Level: ")
selected_grade_level = tk.StringVar()
possible_grade_levels = (7, 8, 9, 10, 11, 12)
grade_level_selector = ttk.Combobox(signup_page, textvariable=selected_grade_level, values=possible_grade_levels, state='readonly')
...
signup_page_widgets = [
    ...
    (account_role_label, 4, 0, 10, 5, "w"),
    (account_role_selector, 4, 1, 10, 5, ""),
    (account_password_label, 7, 0, 10, 5, "w"),
    (account_password_entry, 7, 1, 10, 5, ""),
    (create_account_button, 8, 1, 10 , 5, ""),
    (signup_back_button, 8, 0, 10, 5, "")
]

for counter in range(9):
    signup_page.rowconfigure(counter, weight=1)
signup_page.rowconfigure(8, minsize=20)

grade_section_label = label_constructor(signup_page, "Section: ")
grade_section_selector = ttk.Combobox(signup_page, textvariable=selected_grade_section, values=[], state='readonly')
...
login_given_name_label = label_constructor(login_page, "Given Name: ")
login_given_name_entry = entry_constructor(login_page, 15)
login_surname_label = label_constructor(login_page, "Surname: ")
login_surname_entry = entry_constructor(login_page, 15)
...
login_page_widgets = [
    ...
    (login_given_name_label, 2, 0, 10, 5, "w"),
    (login_given_name_entry, 2, 1, 10, 5, ""),
    (login_surname_label, 3, 0, 10, 5, "w"),
    (login_surname_entry, 3, 1, 10, 5, ""),
    (login_password_label, 4, 0, 10, 5, "w"),
    (login_password_entry, 4, 1, 10, 5, ""),
    (login_account_button, 5, 0, 10, 5, ""),
    (login_back_button, 5, 1, 10, 5, "")
]
...
menu_frame_create_attendance_buttonn = button_constructor(teacher_frame, "Create Attendance", "NORMAL", "TButton", demo_msg)
menu_frame_attendance_button = button_constructor(teacher_frame, "Attendance Dashboard", "NORMAL", "TButton", demo_msg)
menu_frame_export_button = button_constructor(teacher_frame, "Export Data", "NORMAL", "TButton", demo_msg)
...
teacher_menu_widgets = [
    ...
    (menu_frame_export_button, 4, 0, 10, 5, ""),
...
pack_widget_loader(password_frame, reset_password_widgets)
