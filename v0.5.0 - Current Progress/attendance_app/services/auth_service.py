...
def process_account(
    ...
    with get_db() as conn:
        ...
        duplicate_account = user_repo.has_duplicate_account(conn, entered_given_name, entered_surname, selected_account_role)
        
        if duplicate_account:
            return helpers.error_response("Input Error", "You cannot register a duplicate account\nPlease recheck your entered account info, and try again.")

        hashed = helpers.hash_password(entered_password).decode("utf-8")
        user_repo.add_account(
            conn, entered_given_name, 
             entered_surname, hashed,
...
# Login
def process_account_login(entered_given_name, entered_surname, entered_password):
    error_type, error_message = validators.check_login_entries(
        entered_given_name, entered_surname, entered_password
    )
    if error_type:
        return helpers.error_response(error_type, error_message)
    
    with get_db() as conn:
        found_account = user_repo.find_account(conn, entered_given_name, entered_surname)
        if not found_account:
            return helpers.error_repsonse(
                "Input Error",
