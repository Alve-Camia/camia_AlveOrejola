...
def check_duplicate_attendance(
    ...
    cur = conn.cursor()
    cur.execute("""
        SELECT grade_level, section, subject, date
        FROM attendance_sessions
        WHERE grade_level=? AND LOWER(section)=LOWER(?) AND LOWER(subject)=LOWER(?) AND date=?
    """, 
    (entered_attendance_grade, entered_attendance_section, 
    entered_attendance_subject, entered_attendance_date))
...
def insert_attendance_session(
    ...
    cur.execute("""
        ...
        (grade_level, section, subject, period, date, created_at, created_by)
        VALUES (?, ?, ?, ?, ?, ?, ?)
