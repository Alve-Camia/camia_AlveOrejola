...
from attendance_app.db import user_repo, db, attendance_repo
...
# ===== v0.5.0 Reworked Attendance Logic =====
def process_attendance_creation(
    ...
    if has_attendance_error:
        return helpers.error_response(error_title, error_message)
    ...
    with get_db() as conn:
        ...
        stored_attendance_id, stored_student_ids = attendance_repo.insert_attendance_session(
            conn, entered_attendance_grade, 
            entered_attendance_section, entered_attendance_subject,
            entered_attendance_date, entered_attendance_period,
            creator_full_name, now
        )
        ...
        for stored_id in stored_student_ids:
            ...
            attendance_repo.insert_attendance_record(
                conn, attendance_id, 
                student_id
            )
        conn.commit()
