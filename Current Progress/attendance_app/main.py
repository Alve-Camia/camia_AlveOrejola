...
# ===== Attendance Create =====
def table_info_loader(table, fetched_table_info):
    for row in table.get_children():
        table.delete(row)

    for info in fetched_table_info:
        row_id = table.insert("", tk.END, values=info)


def read_attendance_input(session_info, attendance_info):
    ...
    current_user_id = session_info.id
    ...
    if not attendance_creation_results.success:
        display_warning(
            attendance_creation_results.error_title,
    ...
    show_dashboard_frame(session_info, attendance_info)
    show_frame(dashboard_frame)
    return
# =====                  =====

# ===== Attendance Dashboard =====
def show_dashboard_frame(session_info, attendance_info):
    show_frame(dashboard_frame)
    reset_attendance_session(attendance_info)

    creator_id = session_info.id
    attendance_dashboard_info = attendance_service.lookup_creator_attendances(creator_id)
    table_info_loader(dashboard_table, attendance_dashboard_info)

def reset_attendance_session(attendance_info):
    ...
    attendance_info.attendance_id = None
    attendance_info.attendance_grade = None
    attendance_info.attendance_section = None

def call_attendance_exporter(session_info):
    teacher_id = session_info.id
    current_datetime = datetime.now().strftime('%Y-%m-%d, %H-%M-%S')
    export_attendance_results = attendance_service.process_attendance_export(teacher_id, current_datetime)

    if not export_attendance_results.success:
        display_warning(
            export_attendance_results.error_title,
            export_attendance_results.error_message
        )
...
def read_selected_attendance(event):
    ...
    load_attendance_marking_table(selected_row, current_attendance)

def load_attendance_marking_table(selected_row, attendance_info):
    ...
    attendance_id = int(dashborad_row_values[0])
    attendance_date = datetime.strptime(dashborad_row_values[1], "%Y-%m-%d")
    attendance_grade = int(dashborad_row_values[2])
    ...
    attendance_info.attendance_id = attendance_id
    attendance_info.attendance_grade = attendance_grade
    attendance_info.attendance_section = attendance_section

    attend_attendance_info.config(
        text=f"""Grade {attendance_grade} - {attendance_section} | {attendance_subject} | Period {attendance_period} | {attendance_date}""")

    populate_attendance_marking_table(attendance_id, attendance_grade, attendance_section)


def populate_attendance_marking_table(attendance_id, attendance_grade, attendance_section):
    attendance_marking_info = attendance_service.fetch_attendance_marking_info(
        attendance_id, attendance_grade, 
        attendance_section
    )

    table_info_loader(attendance_table, attendance_marking_info)
...
def read_entered_attendance_marking(attendance_info: attendance_session) -> None:
...
def record_attendance_marking(
        ...
        cutting_minutes: str, 
        attendance_id: int,
        attendance_info: attendance_session
    ) -> None:

    attendance_marking_results = attendance_service.process_attendance_marking(
        selected_student_id, attendance_punctuality,
        tardy_minutes, cutting_minutes, 
        attendance_id
    )
    
    if not attendance_marking_results.success:
        display_warning(attendance_marking_results.error_title, attendance_marking_results.error_message)
        return
    
    populate_attendance_marking_table(
    attendance_info.attendance_id, attendance_info.attendance_grade, 
    attendance_info.attendance_section)

def delete_all_attendance_made(session_info):
    ...
    if yes_or_no("Confirm Action", "Are you sure you want to delete all the attendances you have made?\nPlease ensure that you have at least exported your attendance data."):
        teacher_id = session_info.id
        attendance_service.process_attendace_deletion(teacher_id)
        display_info("Info", "All attendance you have made are now deleted.")

# Student Attendance Feature
def show_student_attendance(session_info):
    ...
    view_attendance_info = attendance_service.fetch_student_attendances(session_info.id)
...
def read_account_creation_fields():
    ...
    account_role = selected_account_role.get()
    entered_password = account_password_entry.get()

    entered_account_registration_info = helpers.AccountRegistration(
        entered_given_name, entered_surname, 
        entered_identifier, grade_level,
        grade_section, account_role,
        entered_password)
    process_account_creation(
        entered_account_registration_info
    )

def process_account_creation(registration_info: helpers.AccountRegistration):

    process_account_results = auth_service.account_registration_processer(registration_info)

    if not process_account_results.success:
        display_warning(
            process_account_results.error_title,
            process_account_results.error_message
        )
        return
    
    display_info("Info", "Account has been created.")
    clear_signup_page()
    return
...
def read_login_entries():
    ...
    process_login(entered_password, entered_identifier, current_session)

def process_login(
        entered_password: str, 
        entered_identifier: str,
        session_info: account_session
    ) -> None:
    ...
    session_info.given_name = account_given_name
    session_info.surname = account_surname
    session_info.role = account_role
    session_info.id = account_id
...
def menu_logout():
    ...
    if log_out_decision:
        ...
        clear_session_info(current_session)
        show_frame(login_page)

def show_signup_page():
    show_frame(signup_page)

def clear_session_info(session_info):
    session_info.given_name = None
    session_info.surname = None
    session_info.role = None
...
def show_respective_menu(session_info):
...
def clear_signup_page():
    ...
    grade_year_label.grid_forget()
    grade_level_selector.grid_forget()
    grade_section_label.grid_forget()
    grade_section_selector.grid_forget()
...
def display_warning(warning_title: str, warning_message: str) -> None:
    messagebox.showwarning(warning_title, warning_message)
    return

def display_error(error_title: str, error_message: str) -> None:
    messagebox.showerror(error_title, error_message)
    return

def display_info(info_title: str, info_message: str) -> None:
    messagebox.showinfo(info_title, info_message)
    return

def yes_or_no(popup_title: str, popup_question: str):
...
def show_section_selector(event):
    ...
    if selected_grade in constants.POSSIBLE_GRADE_SECTIONS:
        grade_section_selector.config(values=constants.POSSIBLE_GRADE_SECTIONS[selected_grade])
        grid_layout_manager([
...
def show_possible_sections(event):
    ...
    selected_attendance_section.set('')
    attendance_section_selector.config(values=constants.POSSIBLE_GRADE_SECTIONS[selected_grade_level])
...
center_x = int(screen_width/2 - constants.WINDOW_WIDTH / 2)
center_y = int(screen_height/2 - constants.WINDOW_HEIGHT / 2)

win.geometry(f"{constants.WINDOW_WIDTH}x{constants.WINDOW_HEIGHT}+{center_x}+{center_y}")
...
grade_level_selector = ttk.Combobox(signup_page, textvariable=selected_grade_level, values=constants.GRADE_LEVELS, state='readonly', width=15)
...
create_account_button = button_constructor(signup_page, "Create Account", "NORMAL", "TButton", read_account_creation_fields)
...
menu_frame_attendance_button = button_constructor(teacher_frame, "Dashboard", "NORMAL", "TButton", lambda: show_dashboard_frame(current_session, current_attendance))
...
attendance_grade_selector = ttk.Combobox(
    attendance_create, textvariable=selected_attendance_grade, 
     values=constants.GRADE_LEVELS, state='readonly', 
     width=15
)
...
attendance_create_button = button_constructor(attendance_create, "Create Attendance", "NORMAL", "TButton", lambda: read_attendance_input(current_session, current_attendance))
...
student_view_attendance_button = button_constructor(student_frame, "View Attendance", "NORMAL", "TButton", lambda: show_student_attendance(current_session))
...
settings_frame_attendance_deletion = button_constructor(settings_frame, "Delete All Attendances", "NORMAL", "TButton", lambda: delete_all_attendance_made(current_session))

setting_menu_btn = ttk.Button(settings_frame, text="Back", width=15, style="TButton", command=lambda: show_respective_menu(current_session))
...
accessibility_frame_default_mode = button_constructor(accessibility_frame, "Default", "NORMAL", "TButton", default_screen)
...
dashboard_refersh = button_constructor(dashboard_frame, "Refresh", "NORMAL", "TButton",  lambda: show_dashboard_frame(current_session, current_attendance))
dashboard_export = button_constructor(dashboard_frame, "Export CSV", "NORMAL", "TButton", lambda: call_attendance_exporter(current_session))
...
dashboard_table.bind("<Double-Button-1>", read_selected_attendance)
...
attendance_apply_changes = button_constructor(attendance_marking_frame, "Apply Changes", "NORMAL", "TButton", lambda: read_entered_attendance_marking(current_attendance))

attendance_back_button = button_constructor(attendance_marking_frame, "Back", "NORMAL", "TButton", lambda: show_dashboard_frame(current_session, current_attendance))
